...
### Security
- Updated dependencies to address vulnerabilities in the `cookie` package
- Implemented resolutions in package.json to force the use of a newer version of the `cookie` package

### Changed
- Updated react-scripts and webpack-dev-server to their latest versions
- Modified package.json to include resolutions for consistent package versions

### Added
- Created .env.example file in the root directory for environment variable template
...
### Changed
- Updated AdminTeamTable and TaskTable to make group names clickable for collapsing/expanding
- Modified TeamHideBtn and StatusHideBtn to have pointer-events set to auto
- Improved user experience by allowing clicks on group names to toggle visibility

## [1.2.0] - 2024-10-10

### Added
- Created TeamHideBtn component for collapsing team tables in AdminTeamTable

### Changed
- Refactored AdminTeamTable to use TeamHideBtn instead of StatusHideBtn
- Optimized team sorting in AdminList component using useMemo
- Improved task filtering in AdminTeamTable

### Fixed
- Improved team order change functionality in AdminTeamTable
- Updated updateTeams function in DataContext for immediate UI updates
- Added dispatch function to DataContext
- Fixed TypeScript error in DataContextProvider

### Removed
- Temporarily removed Goals from the navigation menu
